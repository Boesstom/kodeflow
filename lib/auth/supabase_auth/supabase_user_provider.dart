...
class KodeflowAppSupabaseUser extends BaseAuthUser {
  KodeflowAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> kodeflowAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = KodeflowAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
